...
namespace TopFashion
{
    public class UserLogic
    {
        ...
        public User GetUser(int id)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                user.Departments = Common.GetDepartmentIds(dt.Rows[0]["Depart"].ToString());
                ...
                if (dt.Rows[0]["Roles"] != null && dt.Rows[0]["Roles"] != DBNull.Value)
                    user.Roles = Common.GetRoleIds(dt.Rows[0]["Roles"].ToString());
                if (dt.Rows[0]["Usergroup"] != null && dt.Rows[0]["Usergroup"] != DBNull.Value)
                    user.Usergroups = Common.GetUserGroupIds(dt.Rows[0]["Usergroup"].ToString());
        ...
        public List<User> GetAllUsers()
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    user.Departments = Common.GetDepartmentIds(dt.Rows[i]["Depart"].ToString(), dl);
                    ...
                    if (dt.Rows[i]["Roles"] != null && dt.Rows[i]["Roles"] != DBNull.Value)
                        user.Roles = Common.GetRoleIds(dt.Rows[i]["Roles"].ToString(), rl);
                    if (dt.Rows[i]["Usergroup"] != null && dt.Rows[i]["Usergroup"] != DBNull.Value)
                        user.Usergroups = Common.GetUserGroupIds(dt.Rows[i]["Usergroup"].ToString(), ul);
    ...
    public static class UserExtensions
    {
        ...
        {
            foreach (int dep in user.Departments)
            {
                if (dep == depart.ID)
                {
                    return true;
        ...
        public static PermissionCollection GetAllPermissionsByUser(this User user)
        {
            ...
            foreach (int r in user.Roles)
            {
                ...
                if (role.Flag)
                {
                    ...
                    foreach (int p in role.Permissions)
                    {
            ...
            foreach (int d in user.Departments)
            {
                Department dep = DepartmentLogic.GetInstance().GetDepartment(d);
                foreach (int r in dep.Roles)
                {
                    Role role = RoleLogic.GetInstance().GetRole(r);
                    if (role.Flag)
                    {
            ...
            foreach (int ug in user.Usergroups)
            {
                UserGroup ugg = UserGroupLogic.GetInstance().GetUserGroup(ug);
                foreach (int r in ugg.Roles)
                {
                    Role role = RoleLogic.GetInstance().GetRole(r);
                    if (role.Flag)
                    {
