...
namespace TopFashion
{
    ...
    public class SecurityHelper
    {
        ...
        private string safeUrl = "../../403Error.htm";
        //Sql注入时,可能出现的sql关键字,可根据自己的实际情况进行初始化,每个关键字由'|'分隔开来
        //private const string StrKeyWord = @"select|insert|delete|from|count(|drop table|update|truncate|asc(|mid(|char(|xp_cmdshell|exec master|netlocalgroup administrators|:|net user|""|or|and";
        private const string StrKeyWord = @"select|insert|delete|from|drop table|update|truncate|and|alter";  //|or 去掉字符
        private const string StrKeyWord2 = @"exec master|netlocalgroup administrators|net user";
        ...
        public SecurityHelper(HttpRequest _request, HttpResponse _response)
        {
        ...
        public SecurityHelper(HttpRequest _request, HttpResponse _response, string _safeUrl)
        {
            this.request = _request;
            this.response = _response;
        ...
        /// <returns>存在SQL注入关键字时返回 true，否则返回 false</returns>
        private bool CheckKeyWord(string _sWord)
        {
            bool result = false;
            //模式1 : 对应Sql注入的可能关键字
            string[] patten1 = StrKeyWord.Split('|');
            string[] patten10 = StrKeyWord2.Split('|');
            //模式2 : 对应Sql注入的可能特殊符号
            ...
            foreach (string sqlKey in patten10)
            {
                if (_sWord.IndexOf(sqlKey) >= 0)
                {
                    //只要存在一个可能出现Sql注入的参数,则直接退出
        ...
        /// <summary>
        /// 执行Sql注入验证，在MasterPage中调用即可覆盖本站全局
        /// </summary>
        public void Check()
        {
