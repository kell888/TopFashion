...
namespace TopFashion
{
    ...
    {        
        ...
        public List<int> GetTaskStageTemplatesByExecutor(string userId)
        {
            List<int> elements = new List<int>();
            string sql = "select ID from TaskStageTemplate where ','+Executors+',' like '%," + userId + ",%'";
            DataTable dt = sqlHelper.Query(sql);
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    elements.Add(Convert.ToInt32(dt.Rows[i]["ID"]));
                }
            }
            return elements;
        }

        public List<int> GetTaskStageTemplatesByApprover(string userId)
        {
            List<int> elements = new List<int>();
            string sql = "select ID from TaskStageTemplate where ','+Approvers+',' like '%," + userId + ",%'";
            DataTable dt = sqlHelper.Query(sql);
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
