...
namespace TopFashion
{
    public class UserGroupLogic
    {
        ...
        public bool UpgradeList(List<UserGroup> list)
        {
            ...
            {
                string sqlStr = "if exists (select 1 from TF_Module where ID=" + ug.ID + ") update TF_UserGroup set Name='" + ug.Name + "', Roles='" + Common.GetRolesStr(ug.Roles) + "', Remark='" + ug.Remark + "' where ID=" + ug.ID + " else insert into TF_UserGroup (Name, Roles, Remark) values ('" + ug.Name + "', '" + Common.GetRolesStr(ug.Roles) + "', '" + ug.Remark + "')";
                try
                {
        ...
        {
            return sqlHelper.Exists("select 1 from TF_UserGroup where Name='" + name + "'");
        }
        ...
        {
            return sqlHelper.Exists("select 1 from TF_UserGroup where ID!=" + myId + " and Name='" + name + "'");
        }
        ...
        public bool ExistsWhere(string where)
        {
            if (!string.IsNullOrEmpty(where))
            {
                ...
                if (!w.StartsWith("where "))
                    w = "where " + w;
                return sqlHelper.Exists("select 1 from TF_UserGroup " + w);
            }
            return false;
        }
    }
