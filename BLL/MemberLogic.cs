...
namespace TopFashion
{
    ...
    {        
        ...
        public List<MemberSimply> GetAllMembersSimply()
        {
            List<MemberSimply> elements = new List<MemberSimply>();
            string sql = "select ID,姓名 from TF_Member order by ID desc";
            DataTable dt = sqlHelper.Query(sql);
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    MemberSimply element = new MemberSimply();
                    element.ID = Convert.ToInt32(dt.Rows[i]["ID"]);
                    element.姓名 = dt.Rows[i]["姓名"].ToString();
                    elements.Add(element);
                }
        ...
        public int AddMember(Member element)
        {
            ...
            string sql = "insert into TF_Member (卡种, 姓名, 性别, 开卡日, 到期日, 卡号, 生日, 电话, 住址, 备注) values (" + cardType + ", '" + element.姓名 + "', " + (int)element.性别 + ", '" + element.开卡日.ToString("yyyy-MM-dd") + "', '" + element.到期日.ToString("yyyy-MM-dd") + "', '" + element.卡号 + "', '" + element.生日.ToString("yyyy-MM-dd") + "', '" + element.电话 + "', '" + element.住址 + "', '" + element.备注 + "'); select SCOPE_IDENTITY()";
        ...
        {
            string sql = "update TF_Member set 卡种=" + element.卡种.ID + ", 姓名='" + element.姓名 + "', 性别=" + (int)element.性别 + ", 开卡日='" + element.开卡日.ToString("yyyy-MM-dd") + "', 到期日='" + element.到期日.ToString("yyyy-MM-dd") + "', 卡号='" + element.卡号 + "', 生日='" + element.生日.ToString("yyyy-MM-dd") + "', 电话='" + element.电话 + "', 住址='" + element.住址 + "', 备注='" + element.备注 + "' where ID=" + element.ID;
        ...
        public bool UpgradeList(List<Member> list)
        {
            ...
            {
                string sqlStr = "if exists (select 1 from TF_Member where ID=" + element.ID + ") update TF_Member set 卡种=" + element.卡种.ID + ", 姓名='" + element.姓名 + "', 性别=" + (int)element.性别 + ", 开卡日='" + element.开卡日.ToString("yyyy-MM-dd") + "', 到期日='" + element.到期日.ToString("yyyy-MM-dd") + "', 卡号='" + element.卡号 + "', 生日='" + element.生日.ToString("yyyy-MM-dd") + "', 电话='" + element.电话 + "', 住址='" + element.住址 + "', 备注='" + element.备注 + "' where ID=" + element.ID + " else insert into TF_Member (卡种, 姓名, 性别, 开卡日, 到期日, 卡号, 生日, 电话, 住址, 备注) values (" + element.卡种.ID + ", '" + element.姓名 + "', " + (int)element.性别 + ", '" + element.开卡日.ToString("yyyy-MM-dd") + "', '" + element.到期日.ToString("yyyy-MM-dd") + "', '" + element.卡号 + "', '" + element.生日.ToString("yyyy-MM-dd") + "', '" + element.电话 + "', '" + element.住址 + "', '" + element.备注 + "')";
                try
                {
        ...
        public DataTable GetMemberData(string where = null)
        {
            DataTable dt = null;
            string w = "";
            if (!string.IsNullOrEmpty(where))
            ...
            string sql = "select * from TF_Member " + w + " order by ID asc";
        ...
        public DataTable GetMembers(string where)
        {
            ...
            string sql = "select * from TF_View_Member " + w + " order by ID desc";
            dt = sqlHelper.Query(sql);
            return dt;
        }
